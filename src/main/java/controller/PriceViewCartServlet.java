...
public class PriceViewCartServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            // Gọi JPA để xác nhận sản phẩm tồn tại (nếu cần)
            EntityManager em = util.JPAUtil.getEntityManagerFactory().createEntityManager();
            try {
                ProductDAO productDAO = new ProductDAO(em);
                Product product = productDAO.getProductByID(rid);
                if (product != null) {
                    cart.removeItem(rid);
                }
            } finally {
            ...
            e.printStackTrace(); // Log nếu lỗi ID
        }
        ...
        // Forward đến trang hiển thị giỏ hàng
    ...
    public String getServletInfo() {
        return "Handles item removal from cart using JPA for product verification";
    }// </editor-fold>

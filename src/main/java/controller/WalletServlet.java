...
public class WalletServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userName == null || valueStr == null || valueStr.trim().isEmpty()) {
        ...
        // Gọi DAO xử lý bằng JPA
        EntityManager em = JPAUtil.getEntityManagerFactory().createEntityManager();
        WalletDAO walletDAO = new WalletDAO(em);
        walletDAO.inputMoney(userName, value); // đã xử lý transaction bên DAO

        // Cập nhật lại thông tin wallet
        Wallet wallet = walletDAO.getWalletByUserName(userName);
        session.setAttribute("wallet", wallet);

        // Quay về profile.jsp (thông qua servlet "profile")
        request.getRequestDispatcher("profile").forward(request, response);
    }
    ...
    public String getServletInfo() {
        return "Handles adding money to user's wallet";
    }// </editor-fold>
